...
const P=fs.readFileSync('qgen-samples.txt','utf8').split('===').map(s=>s.trim());
let tot=0,ok=0; const cov={}; L.forEach(l=>cov[l]=0); const per={}; let mc=0, num=0;
P.forEach((p,i)=>{ const r=Q.generate(p,{seed:i+1}); const got=new Set();
  r.questions.forEach(q=>{ tot++; const pr=AI.predict(m,q.stem); if(pr.level===q.level) ok++; got.add(q.level); per[q.level]=(per[q.level]||0)+1; if(q.type==='mc') mc++; if(q.numeric) num++;
