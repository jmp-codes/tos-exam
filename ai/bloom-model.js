...
const BloomAI = (() => {
  ...
  const READER = [
    ...
    K("Analyzing","Multiple choice that asks you to differentiate, find assumptions, causes or patterns",
      /^(which|what)\b[^?]*\b(does not belong|doesn't belong|is the odd one|distinguish(es)?|differentiates?|underl(ies|ying)|assumptions?|conclusion (can|could) be (drawn|made)|can be (inferred|concluded)|contributed (most|the most)|pattern (can be|is) (observed|seen)|most (directly )?affected|pattern in|best explains the (relationship|difference|connection)|describes the relationship|relationship between|how are\b[^?]*\b(related|connected))\b|^how (are|is)\b[^?]*\b(related|connected) (to|with)\b|\bwhat (happens|will happen) to (the )?\w+[^?]*\bif\b[^?]*\b(doubled|halved|tripled|increased|decreased)\b/i),
