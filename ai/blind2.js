...
for(const f of ['hard.tsv','blind.tsv']) fs.readFileSync(f,'utf8').trim().split('\n').forEach(l=>{const [lv,t]=l.split('\t'); ex.push({text:t,level:lv});});
const model=AI.train(ex,{epochs:40,lr:0.5,l2:0.001});
const hard=fs.readFileSync('blind2.tsv','utf8').trim().split('\n').map(l=>l.split('\t'));
...
const n=hard.length;
console.log(`Blind set 2 (${n}): Our AI 6-level ${(100*a6/n).toFixed(0)}%  3-column ${(100*a3/n).toFixed(0)}%  | Keywords 6-level ${(100*k6/n).toFixed(0)}%  3-column ${(100*k3/n).toFixed(0)}%`);
